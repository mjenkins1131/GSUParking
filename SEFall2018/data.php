<?php
		...
		if(isset($_POST["Type"]))
		{
			...
			if($type == "Get")
			{
				if(isset($_POST["Target"]))
				{
					...
					else if($target == "CampusesLotHTML"){
						echoCampusesLotHTML($conn);
					}
					else if($target == "TrendsHTML")
					{
						$campusID = $_POST["CampID"];
						echoTrendsDivCount($conn,$campusID);
					}
					...
					else if($target == "AverageDays"){
						$lotID = $_POST["LotID"];
						$daysID = $_POST["Day"];
						echoLotAverageDays($conn,$lotID,$daysID);
					}
				}
				else
	...
	function echoCampusesLotHTML($conn){
		$i = 1;
		$l = 1;
			$sql = "SELECT * FROM Organizational_Unit ORDER BY Organization_ID ASC";
		$rs = $conn->query($sql);
		...
		if($rs)
		{
			$ar[0]["Campus"] = "<option value='0' selected disable selected hidden>Choose Campus</option>";
			while($row = $rs->fetch_assoc()){
				$ar[$i]["Campus"] = "<option value='".$row["Organization_ID"]."'>".$row["Organization_Name"]."</option>";
				$i++;
			}
		}
		else
		...
		if($rs)
		{
			$ar[0]["Lot"] = "<option value='Choose Lot' selected disable selected hidden>Choose Lot</option>";
			while($row = $rs->fetch_assoc()){
				$ar[$l]["Lot"] = "<option value='".$row["Lot_ID"]."'>".$row["LotNumber"]."</option>";
			$l++;
			}
		}
		else
	...
	function echoLotsHTML($conn, $campusID)
	{
		...
		if($rs)
		{
			echo "<option value='Choose Lot' selected disable selected hidden>Choose Lot</option>";
			while($row = $rs->fetch_assoc())
	...
	function echoTrendsDivCount($conn,$campusID){
	$i = 0;
	$sql = "SELECT * FROM Lot WHERE Organization_ID = ".$campusID;
	$rs = $conn->query($sql);
	if($rs){
		...
		while($row = $rs->fetch_assoc()){
		if($i == 0)
		$ar["RowCount"] = mysqli_num_rows($rs);
		$ar[$i]["LotName"] = $row["LotNumber"];
		$i++;
		}
		echo json_encode($ar);
	}
	else
		echo("get lot count failed");
	...
	function echoLotAverageDays($conn,$lotID,$daysID){
		$sql = "SELECT * FROM Stat WHERE Lot_ID = ".$lotID." and "."dayofweek(Date_Time) = ".$daysID;
		$rs = $conn->query($sql);
		if($rs)
		{
			$eight = 0;
